...
class GraphDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        for v_filename in self.raw_paths:
            ...
            prefix = v_filename.replace('_v.txt', '')
            m = np.loadtxt(f'{prefix}_attn_per_joint.txt')
            tpl_e = np.loadtxt(f'{prefix}_tpl_e.txt').T
            geo_e = np.loadtxt(f'{prefix}_geo_e.txt').T
            joints = np.loadtxt(f'{prefix}_j.txt')
            adj = np.loadtxt(f'{prefix}_adj.txt', dtype=np.uint8)
            vox_file = f'{prefix}.binvox'

            with open(vox_file, 'rb') as fvox:
                vox = binvox_rw.read_as_3d_array(fvox, fix_coords=False)
            ...
            name = os.path.basename(v_filename).split('_')[0]
            v = torch.from_numpy(v).float()
            m = torch.from_numpy(m).long().T
